...
class Common {
	...
	public function advanced_search_autosuggest_handler() {
		...
		$form_id = sanitize_text_field($requested_data['form_id']);
		...
		$search_form_settings = $wpdb->get_results("SELECT data FROM $search_form_table where id='$form_id'");
		...
		if($wpdb->num_rows > 0) {
			...
			if(!empty($t_data)) {
				foreach ($t_data as $final) {
					if($final->post_type == 'attachment' || $final->post_type == 'revision' || $final->post_status == 'revision') {
						...
						$html .= '<div class="page_post"><a href="'.esc_url(get_the_permalink($final->ID)).'">'.esc_attr__($final->post_title, $this->plugin_text_domain).' ('.$final->post_type.')</a> </div>';
					}
				}
			}
			...
			$taxonomies_name = '"'.implode('","', $search_form_settings['taxonomies']).'"';
			...
			if(array_key_exists('search_areas', $search_form_settings['taxonomies'])) {
				...
				if($count > 0){
				    foreach ($terms_result as $term) {
						$html .= "<div><a href='".esc_url(get_term_link( $term ))."'>".esc_attr__($term->name, $this->plugin_text_domain). " (".$term->taxonomy.")</a></div>";
				 	}
				}
				...
				if(!empty($attachments_data)) {
					foreach ($attachments_data as $attach_final) {
						if($attach_final->post_title != '') {
							...
							$html .= '<div class="attachments"><a href="'.esc_url($link).'" target="_blank">'.esc_attr__($attach_final->post_title, $this->plugin_text_domain).' ('.$attach_final->post_type.')</a> </div>';
						}
					}
				}
			...
			if($html == '') {
				$html .= '<div class="not_found">'.esc_attr__('No results found.', $this->plugin_text_domain).'</div>';
			}
	...
	public function shortcode_WPAS_Advanced_Search( $atts, $content = null ) {
		...
		$search_forms = $wpdb->get_results("SELECT id, name, data FROM $search_form_table where id='$form_id'");
	...
	<form id="wpas-advanced-search-form_'.$form_id.'" role="search" method="POST" class="wpas_search_form search_form_'.$form_id.'" action="" onkeydown="return event.key != \'Enter\';">
...
<div class="wpas_search_close">
	...
	public function default_theme_search_form_setting() {
		...
		global $wpdb;
		$search_form_table = $wpdb->prefix."wpas_index";
		$search_form_setting = $wpdb->get_results("SELECT * FROM $search_form_table where id='$default_search_form_id'");
